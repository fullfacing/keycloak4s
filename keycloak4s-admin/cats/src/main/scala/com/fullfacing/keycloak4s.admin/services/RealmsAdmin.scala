...
class RealmsAdmin[R[+_]: Concurrent, S](implicit client: KeycloakClient[R, S]) {
  ...
  def enableUsersManagementPermissions(realm: String = client.realm): R[Either[KeycloakError, ManagementPermission]] = {
    val path = Seq(realm, "users-management-permissions")
    client.put[ManagementPermission](path, ManagementPermission.Enable(true))
  }

  /** Disables a realm's user management permissions. */
  def disableUsersManagementPermissions(realm: String = client.realm): R[Either[KeycloakError, ManagementPermission]] = {
    val path = Seq(realm, "users-management-permissions")
