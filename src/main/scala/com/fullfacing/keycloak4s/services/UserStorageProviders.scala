...
class UserStorageProviders[R[+_]: Concurrent, S](implicit client: KeycloakClient[R, S]) {
   ...
   * @return
   */
  def userSimpleProviderName(realm: String, userStorageId: String): R[Either[KeycloakError, SimpleNameResponse]] = {
   ...
   * @return
   */
  def removeImportedUsers(realm: String, userStorageId: String): R[Either[KeycloakError, Unit]] = {
   ...
   * @return
   */
  def syncUsers(realm: String, userStorageId: String, action: Option[String]): R[Either[KeycloakError, Synchronization]] = {
   ...
   * @return
   */
  def unlink(realm: String, userStorageId: String): R[Either[KeycloakError, Unit]] = {
   ...
   * @return
   */
  def syncMapperData(realm: String, mapperId: String, userStorageId: String, direction: Option[String]): R[Either[KeycloakError, Synchronization]] = {
